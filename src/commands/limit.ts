...
async function execute(interaction: CommandInteraction<CacheType>) {
    ...
    const limit = interaction.options.get('number', true);
    if (!limit) {
        await interaction.reply({ ephemeral: true, content: 'You must provide a limit' });
        return;
    }
    const limitValue = limit.value as number;
    if (limitValue < 0) {
        await interaction.reply({ ephemeral: true, content: 'Limit must be greater than 0' });
        return;
    }
    ...
    await voiceChannel.setUserLimit(limitValue);
    if (limitValue === 0) {
        await interaction.reply({ ephemeral: true, content: 'User limit removed' });
    } else {
        await interaction.reply({ ephemeral: true, content: `Limit set to ${limitValue}` });
    }
}
